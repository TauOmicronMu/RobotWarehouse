...
public class TSP {
    ...
    /**
     *
     * @param startNode The user's chosen starting node, route must start and
     * finish here
     * @param destinations The list of Nodes which the user has chosen to visit
     * @param listOfNodes The list of every Node in the network
     */
    public void getRoute(Node startNode, List<Node> destinations, List<Node> listOfNodes) {
        int numberOfNodes = destinations.size();
        routeMatrix = new ArrayList[numberOfNodes][numberOfNodes];
        routeMatrix = initiateRouteMatrix(routeMatrix);
        //stores the weights between each node
        double[][] adjacencyMatrix = new double[numberOfNodes][numberOfNodes];
        adjacencyMatrix = initiateMatrix(adjacencyMatrix);
        adjacencyMatrix = adjMatrix(adjacencyMatrix, destinations, listOfNodes);
        ...
        List<List<Pair>> currentGroups = new ArrayList<>();
        ...
        getShortestRoute(adjacencyMatrix, route, edgesLeft, edgesConnected, 0, 1, currentGroups);

        //finds the final route through the network
        getFinalRoute(startNode, destinations);
    }
        ...
        for (int Node1 = 0; Node1 < destinations.size() - 1; Node1++) {
            for (int Node2 = Node1 + 1; Node2 < destinations.size(); Node2++) {
                ...
                for (Node foundNode : foundRoute) {
                    routeMatrix[Node1][Node2].add(foundNode);
                }
     ...
     *
     * @param routeMatrix Stores the route taken between each destination
     ...
     * @param adjacencyMatrix The adjacency matrix which stores all of the
     * connections between my nodes
     * @param route The current route for this subset of possible routes
     * @param edgesLeft The amount of edges still available to each specific
     * node
     * @param edgesConnected The amount of edges connected to each specific node
     * @param currentNode The value of the current node which the connections
     * are being decided for
     * @param connectedNode The value of the node which the current node is
     * either being connected to or removed from
     * @param currentGroups The list of groups (separate sections of route)
     * which are involved in this subset of possible routes
     */
    private void getShortestRoute(double[][] adjacencyMatrix, List<Pair> route, int[] edgesLeft, int[] edgesConnected, int currentNode, int connectedNode, List<List<Pair>> currentGroups) {
        ...
        // checks impact of adding or removing nodes on the adjacency matrix and modifies accordingly
        for (int x = 0; x < matrices.length; x++) {
            ...
            //removes any possible remaining connections to nodes which are already fully connected
            ...
            // check to see if an edge must be added as it would not be possible for either node to have two edges without it
            for (int i = 0; i < edgesLeftNew.length; i++) {
                // finds how many edges are left unconnected, if there are 2 left unconnected and there are
                // currently none connected to that node, then connect both of them.
                // if there is only one edge left and one connected then add the last edge
                if (edgesLeftNew[i] == 2) {
                    if (currentConnectedEdges[i] == 0) {
                        //find both of the edges which need to be added to the route
                        for (int linkedNode = 0; linkedNode < currentMatrix.length; linkedNode++) {
                            boolean notAdded = addedCheck(currentRoute, i, linkedNode);
                            if (notAdded && currentMatrix[i][linkedNode] != Double.POSITIVE_INFINITY) {
                ...
                for (int checkNode1 = currentNode; checkNode1 < adjacencyMatrix.length - 1; checkNode1++) {
                    for (int checkNode2 = checkNode1 + 1; checkNode2 < adjacencyMatrix.length; checkNode2++) {
                        ...
                        if (notAdded && currentMatrix[checkNode1][checkNode2] != Double.POSITIVE_INFINITY) {
                            ...
                            // check to see if edge would create a node with more than two edges or create a cycle
                            ...
                            // if the check found an edge which could be removed, then remove it from matrix and list of edges left
            ...
            for (int i = 0; i < edgesLeftNew.length; i++) {
                if (edgesLeftNew[i] == 2) {
                    if (currentConnectedEdges[i] == 0) {
                        //finds the two remaining edges which need to be added to the route
                        for (int linkedNode = 0; linkedNode < currentMatrix.length; linkedNode++) {
                            boolean notAdded = addedCheck(currentRoute, i, linkedNode);
                            if (notAdded && currentMatrix[i][linkedNode] != Double.POSITIVE_INFINITY) {
                                // adds edge to route
            ...
            // updates the arrays for both halves of split with new values for this half
            // arrays are copied manually to avoid errors occurring from being passed by reference
        ...
        // deals with the two different routes in order of lowest lower bound first
                ...
                if (complete) {
                    ...
                    if (lowerBounds[split] < lowestWeight) {
                        ...
                        if (complete) {
                            ...
                            // prune other route as this is now the shortest length
     ...
     *
     * @param routes The two different routes which are created by splitting the
     * previous route into two subsets.
     * @param matrices The two adjacency matrices made by splitting the previous
     * route into subsets
     * @param allEdges The two different lists containing the amount of edges
     * already connected for each node for both splits
     * @param allEdgesLeft The two different lists containing the amount of
     * edges still available for each node for both splits
     * @param currentNode The value of the current node which the connections
     * are being decided for
     * @param connectedNode The value of the node which the current node is
     * either being connected to or removed from
     * @param split A number which states which half of the split is being
     * passed down
     * @param groupsArray The two different lists of groups for both splits
     */
    private void call(List<List<Pair>> routes, double[][][] matrices, int[][] allEdges, int[][] allEdgesLeft, int currentNode, int connectedNode, int split, List<List<Pair>>[] groupsArray) {
        ...
        // if it has reached the end of a row in the adjacency matrix, then increment current node and set connected node to currentNode +1;
        Boolean available;
        do {
        ...
        getShortestRoute(currentMatrix, currentRoute, edgesLeftNew, currentConnectedEdges, currentNode, connectedNode, currentGroups);
    ...
    private boolean addedCheck(List<Pair> route, int currentNode, int connectedNode) {
        ...
        for (Pair p : route) {
            if ((p.getNode1() == currentNode && p.getNode2() == connectedNode) || (p.getNode1() == connectedNode && p.getNode2() == currentNode)) {
                available = false;
            }
        }
        return available;
     ...
     *
     * @param position An array which contains both nodes from all the pairs in
     * the best route found separately
     * @param nodeToFind The node which is being looked for in this round of
     * recursion
     * @param destinations The list of Nodes which the user has chosen to visit
     */
     ...
     * the best route found separately
     * @param destinations The list of Nodes which the user has chosen to visit
     * @param x The value for which Pair location in the position array the node
     * was found in
     * @param y The value for which Node location in the position array the node
     * was found in
     * @param z The value for which Node location in the position array the next
     * node to find is in
    ...
    private Node[][] addRoute(Node[][] position, List<Node> destinations, int x, int y, int z) {
        ...
        List<Node> foundRoute = routeMatrix[findLocation(position[x][y], destinations)][findLocation(position[x][z], destinations)];
     ...
     *
     * @param n The node to find the location of
     * @param destinations The list of Nodes which the user has chosen to visit
     ...
     * @param currentGroups The list of groups (separate sections of route)
     * which are involved in this subset of possible routes
     * @param pairToAdd The Pair which needs to be added to a group
        ...
        if (currentGroups.isEmpty()) {
            ...
            for (int i = 0; i < currentGroups.size(); i++) {
                for (Pair p : currentGroups.get(i)) {
                    if (p.getNode1() == pairToAdd.getNode1() || p.getNode1() == pairToAdd.getNode2() || p.getNode2() == pairToAdd.getNode1() || p.getNode2() == pairToAdd.getNode2()) {
